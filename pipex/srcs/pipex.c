...
/*   Updated: 2022/02/16 20:31:33 by rcuminal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_pipex(t_val *val, char **env, int fd, pid_t pid)
{
	...
	if (ft_strncmp(val->cmd1[0], val->path, ft_strlen(val->path)) == 0)
...
void	ft_pipexalone(t_val *val, char **env, char **cmd, pid_t pid)
{
	...
	if (ft_strncmp(val->cmd2[0], val->path, ft_strlen(val->path)) == 0)
	{
		write(2, "command not found: ", 20);
		ft_putstr_fd(val->path, 2);
		return ;
	}
	pipe(val->pfd);
	pid = fork();
